...
- Make a list of files here.
  > `ls8.py`, `cpu.py`, `call.ls8`
- Write a short 3-10-word description of what each file does.
  > `cpu.py` file holds the main functionality of our `CPU`.
  > `ls8.py` file is going to be our 8-bit emulator that creates, loads and runs our `CPU`.
  > `call.ls8` file is going to `load` `r1` with `MULT2PRINT` > load R0 with 10 > call on R1 > load R0 with 15 > call R1 > load R0 with 18 > call R1 again > load R0 with 30 > call R1 again > HALT + exit > since we halted at line 24 in the stack we leave our R1 value at memory address 24 > add (R0 + R0) > Print R0 > Return/Exit emulator.
